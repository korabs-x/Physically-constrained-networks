...
def mp_worker(data):
    for n_train in train_range:
        ...
        if lossfnstr == 'det' or lossfnstr == 'det_variable' or lossfnstr == 'detnorm_variable':
            lossfns[0]['loss_fn'].append({'loss_fn': lossfn.get_det_loss(), 'weight': 0.5, 'label': 'det'})
        if lossfnstr == 'norm' or lossfnstr == 'norm_variable' or lossfnstr == 'detnorm_variable':
            lossfns[0]['loss_fn'].append({'loss_fn': lossfn.get_norm_loss(), 'weight': 0.5, 'label': 'norm'})
...
def mp_handler():
    for lossfnstr in loss_fns:
        p = multiprocessing.Process(target=mp_worker, args=(lossfnstr,))
        p.start()
